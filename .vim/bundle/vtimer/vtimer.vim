...
function! Vtimer_leave()
	...
	let s:v_file = readfile($HOME.'/.vim/bundle/vtimer/vtimer.vim')
	let s:v_total = str2nr(strpart(s:v_file[19],1))
	let s:v_total += s:v_add
	let s:v_file[19] = '"'.s:v_total
	call writefile(s:v_file, $HOME.'/.vim/bundle/vtimer/vtimer.vim')
endfunction

function! Vtimer_show()
	let s:v_add = localtime() - s:v_start
	let s:v_total = str2nr(strpart(readfile($HOME.'/.vim/bundle/vtimer/vtimer.vim')[19],1))
